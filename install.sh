#!/bin/bash
# =============================================================================
# HestiaCP → Bunny DNS Plugin – install.sh v2.3.0
...
# ── HestiaCP hooks ────────────────────────────────────────────────────────────
# Hooks fire synchronously inside HestiaCP commands, guaranteeing that Bunny
# is updated even when inotifywait misses the initial creation event (race
# condition with newly-created directories in recursive watch mode).
#
# v-add-dns-domain   → sync new zone to Bunny immediately
# v-add-domain       → safety net when DNS is part of a full domain add
# v-delete-dns-domain → delete zone from Bunny on DNS removal
# v-delete-domain    → delete zone from Bunny on full domain removal
# v-delete-user      → bulk-delete all Bunny zones owned by a removed user
